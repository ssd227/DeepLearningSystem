...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if prod(self._shape) != prod(new_shape): 
            raise ValueError("shape not match or not compact")
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        if new_axes == tuple(range(self.ndim)):
            return self
    ...
    def broadcast_to(self, new_shape):
        ...
        """
        ### BEGIN YOUR SOLUTION
        assert all([new_shape[i] == self.shape[i] or self.shape[i] == 1 for i in range(self.ndim)])
        new_strides = tuple([self._strides[i] if new_shape[i]== self.shape[i] else 0 for i in range(self.ndim)])
        return self.as_strided(shape=new_shape, strides=new_strides)
        ### END YOUR SOLUTION
