...
class RandomCrop(Transform):
    ...
    def __call__(self, img):
        ...
        padding_img = np.zeros((H+2*P, W+2*P, C))
        padding_img[P:P+H, P:P+W,:] = img

        nx = P + shift_x
        ny = P + shift_y

        return padding_img[nx:nx+H, ny:ny+W,:] #crop
        ### END YOUR SOLUTION
...
class DataLoader:
    ...
    def __next__(self):
        ...
        if self.batch_id >= len(self.ordering):
          raise StopIteration()
        batch_data = self.dataset[self.ordering[self.batch_id]]
        self.batch_id +=1
        return [Tensor(s) for s in batch_data]
        ### END YOUR SOLUTION
...
class MNISTDataset(Dataset):
    def __init__(
        ...
        self.images = images_normalized.reshape((-1, 28, 28, 1))
        self.labels = labels
        self.transforms = transforms
        ### END YOUR SOLUTION
...
class CIFAR10Dataset(Dataset):
    def __init__(
        ...
        # reshape and norm
        X = X.reshape((X.shape[0], 3, 32, 32)).astype("float32") / np.float32(255.0)
...
def batchify(data, batch_size, device, dtype):
    ...
    for i in range(nbatch):
        arr.append(data[i*batch_size: (i+1)*batch_size])
    return np.array(arr, dtype=dtype).reshape(nbatch, batch_size)
    ### END YOUR SOLUTION
...
def get_batch(batches, i, bptt, device=None, dtype=None):
    ...
    target = batches[i+1:i+1+seq_len].reshape(-1) # todo 这里的target reshape是几个意思？ (bptt*bs,)
