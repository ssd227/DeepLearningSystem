...
class SparseMatrix:
    def __init__(self, m, n, nnz, device=None):
        self._device = device if device is not None else default_device()
        self._m = m
        self._n = n
        self._nnz = nnz
        self._handle = self._device.SparseMatrix(m, n, nnz)
...
class NDArray:
    ...
    def broadcast_to(self, new_shape):
        ...
        """
        ### BEGIN YOUR SOLUTION
        assert all([new_shape[i] == self.shape[i] or self.shape[i] == 1 for i in range(len(new_shape))])
    ...
    def __getitem__(self, idxs):
        ...
        assert len(idxs) == self.ndim, f"Need indexes equal to number of dimensions: {len(idxs)} != {self.ndim}"

        ### BEGIN YOUR SOLUTION
        new_shape = tuple([slice_len_for(idx) for idx in idxs])
        new_strides = tuple([self.strides[i] * idx.step for i, idx in enumerate(idxs)])
        new_offset = self._offset + sum([self.strides[i] * idx.start for i, idx in enumerate(idxs)])
        res = self.as_strided(new_shape, new_strides)
        res._offset = new_offset
        return res
        ### END YOUR SOLUTION
    ...
    def reduce_view_out(self, axis, keepdims=False):
        ...
        # print("reduce_view_out axis: ", axis)
        if axis is None:
    ...
    def flip(self, axes):
        ...
        ### BEGIN YOUR SOLUTION
        origin = self.compact()
        stride = list(self._strides)
        offset = self._offset
        for axis in axes:
            offset += (self.shape[axis] - 1) * stride[axis]
            stride[axis] *= -1
        stride = tuple(stride)
        out = NDArray.make(self.shape, device=self.device, strides=stride, offset=offset, handle=origin._handle).compact()
        return out
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        out.fill(0)
        slicing = []
        for (l, r), s in zip(axes, self.shape):
    ...
    def dilate(self, axes: tuple, dilation: int):
        """
        Dilate this ndarray by the specified dilation in `axes`.
        """
        ### BEGIN YOUR SOLUTION
        out_shape = tuple([s * (dilation+1) if i in axes else s for i, s in enumerate(self.shape)])
        out = NDArray.make(out_shape, device=self.device)
        out.fill(0)
        slicing = []
        for i, s in enumerate(self.shape):
            if i in axes:
                slicing.append(slice(None, None, dilation+1))
            else:
    ...
    def undilate(self, axes: tuple, dilation: int):
        """
        Undilate this ndarray by the specified dilation in `axes`.
        """
        ### BEGIN YOUR SOLUTION
        out_shape = tuple([s // (dilation+1) if i in axes else s for i, s in enumerate(self.shape)])
        out = NDArray.make(out_shape, device=self.device)
...
def to_sparse(a: NDArray):
    """
    Convert this ndarray to a sparse matrix.
    """
    ### BEGIN YOUR SOLUTION
    nnz = np.count_nonzero(a.numpy())
    out = SparseMatrix(a.shape[0], a.shape[1], nnz, device=a.device)
    a.device.to_sparse(a._handle, out._handle, a.shape[0], a.shape[1], nnz)
    return out
    ### END YOUR SOLUTION

def to_dense(a: SparseMatrix):
    """
    Convert this sparse matrix to a dense ndarray.
    """
    ### BEGIN YOUR SOLUTION
    out = NDArray.make((a._m, a._n), device=a._device)
    a._device.to_dense(a._handle, out._handle)
    return out
    ### END YOUR SOLUTION

def sparse_transpose(a: SparseMatrix):
    """
    Transpose this sparse matrix.
    """
    ### BEGIN YOUR SOLUTION
    out = SparseMatrix(a._n, a._m, a._nnz, device=a._device)
    a._device.sparse_transpose(a._handle, out._handle)
    return out
    ### END YOUR SOLUTION

def spmm(a: SparseMatrix, b: NDArray):
    """
    Sparse matrix times dense matrix.
    """
    ### BEGIN YOUR SOLUTION
    assert a._n == b.shape[0]
    out = NDArray.make((a._m, b.shape[1]), device=a._device)
    a._device.spmm(a._handle, b.compact()._handle, out._handle, a._m, a._n, b.shape[1])
    return out
    ### END YOUR SOLUTION