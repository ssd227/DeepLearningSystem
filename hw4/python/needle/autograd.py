...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    #   print("sgd-log",type(vi.grad))
      assert isinstance(vi.grad, (Tensor,TensorTuple))
      vk_out_grad_list = vi.op.gradient_as_tuple(vi.grad, vi)
      assert len(vk_out_grad_list) == len(vi.inputs)

      # trans gradient form vi_out_grad to vi_input node vk
      for vk, vk_p_out_grad in zip(vi.inputs, vk_out_grad_list):
        # add item to dict {key(Tensor):list[Tensor]}
        if vk not in node_to_output_grads_list:
          node_to_output_grads_list[vk]= [vk_p_out_grad]
        else:
          node_to_output_grads_list[vk].append(vk_p_out_grad)
    ### END YOUR SOLUTION
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    for child_node in node.inputs:
      topo_sort_dfs(child_node, visited, topo_order)
    visited.add(node)
